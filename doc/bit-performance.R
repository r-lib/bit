...
Domain <- c(small=1e2, big=1e3)
Sample <- c(small=1e2, big=1e3)
#Domain <- c(small=1e1, big=1e2)
#Sample <- c(small=1e1, big=1e2)
times <- 5
...
## ---- echo=FALSE, fig.cap = "% size and execution time for bit (b) and bitwhich (w) relative to logical in the 'rare' scenario"----
oldpar <- par(xaxs="i")
x <- tim[1:3,,"rare"]
dotchart(x, xlim=c(0,max(100, max(x))), labels="", pch=c("l","b","w"), col=c("black","blue","red"), main="% size and timings in 'rare' scenario", sub="l='logical'  b='bit'  w='bitwhich'           % of 'logical'")
par(oldpar)

## ---- echo=FALSE, fig.cap = "% size and execution time for bit (b) and bitwhich (w) relative to logical in the 'coin' scenario"----
oldpar <- par(xaxs="i")
x <- tim[1:3,,"often"]
dotchart(x, xlim=c(0,max(100, max(x))), labels="", pch=c("l","b","w"), col=c("black","blue","red"), main="% size and timings in 'often' scenario", sub="l='logical'  b='bit'  w='bitwhich'           % of 'logical'")
par(oldpar)

## ---- echo=FALSE, fig.cap = "% size and execution time for bit (b) and bitwhich (w) relative to logical in the 'coin' scenario"----
oldpar <- par(xaxs="i")
x <- tim[1:3,,"coin"]
dotchart(x, xlim=c(0,max(100, max(x))), labels="", pch=c("l","b","w"), col=c("black","blue","red"), main="% size and timings in 'coin' scenario", sub="l='logical'  b='bit'  w='bitwhich'           % of 'logical'")
par(oldpar)
...
## ---- echo=FALSE, results='asis'-----------------------------------------
knitr::kable(round(tim[,"[]",], 1), caption="% bytes of logical")

## ---- echo=FALSE, results='asis'-----------------------------------------
knitr::kable(round(tim[,"[]<-logical",], 1), caption="% bytes of logical")

## ---- echo=FALSE, results='asis'-----------------------------------------
knitr::kable(round(tim[,"[which]",], 1), caption="% bytes of logical")

## ---- echo=FALSE, results='asis'-----------------------------------------
knitr::kable(round(tim[,"[which]<-TRUE",], 1), caption="% bytes of logical")
...
x <- timsort[,,,"big"]
#x <- 100 * x / max(x["R",,], na.rm=TRUE)
oldpar <- par(mfrow=c(2,1), mar=c(5,8,2,1), xaxs="i")
dotchart(x[,,"unsorted"], xlim=c(0, max(100, max(x))), xlab="execution time", main="unsorted", pch=15:16, col=c("red","blue"))
dotchart(x[,,"sorted"], xlim=c(0, max(100, max(x))), xlab="execution time", main="sorted", pch=15:16, col=c("red","blue"))
par(oldpar)
...
oldpar <- par(xaxs="i")
x <- tim[,,"bigbig","unsorted"]
dotchart(x, xlim=c(0,max(100,max(tim[,,"bigbig",], na.rm=TRUE))), labels="", pch=c("R","b","m"), col=c("red","blue","black"), main="Timings in 'unsorted bigbig' scenario", sub="R='hash'   b='bit'   m='merge'")
par(oldpar)

## ---- echo=FALSE, fig.cap = "Execution time for R, bit and merge in 'sorted bigbig' scenario"----
oldpar <- par(xaxs="i")
x <- tim[,,"bigbig","sorted"]
dotchart(x, xlim=c(0,max(tim[,,"bigbig",], na.rm=TRUE)), labels="", pch=c("R","b","m"), col=c("red","blue","black"), main="Timings in 'sorted bigbig' scenario", sub="R='hash'   b='bit'   m='merge'")
par(oldpar)
